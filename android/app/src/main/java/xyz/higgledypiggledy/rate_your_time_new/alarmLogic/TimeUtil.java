...
public class TimeUtil {
  ...
  public static Calendar nextOccurrence(
    ...
    Log.d(TAG, "nextOccurrence() called with: now = [" + now + "], secondsPastMidnight = [" + secondsPastMidnight + "], repeat = [" + repeat + "]");
    if (then.before(now))
      then.add(Calendar.DATE, 1);
    while (repeat > 0 && !dayIsRepeat(then.get(Calendar.DAY_OF_WEEK), repeat)) {
      Log.d(TAG, "nextOccurrence: ADDING IN THEN BECAUSE OF REPEAT");
      then.add(Calendar.DATE, 1);
    }
    Log.d(TAG, "nextOccurrence: RETURNING THEN WITH DATE + "+then.get(Calendar.DATE));
    Log.d(TAG, "nextOccurrence: ---------------------------------------------------------------------------------------------");
    return then;
  }
  ...
  static final int EVERYDAY = 1 | 2 | 4 | 8 | 16 | 32 | 64;
