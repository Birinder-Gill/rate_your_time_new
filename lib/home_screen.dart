...
import 'widgets/date_pickers/date_picker_widget.dart';
  ...
  void initState() {
    ...
    model.animController?.addStatusListener((status) {
      if (status == AnimationStatus.completed ||
        ...
        _tabIndex.value = model.animController?.value??0;
      }
    });
    ...
    model.expandingController?.addStatusListener((status) {
      if (status == AnimationStatus.completed ||
          status == AnimationStatus.dismissed) {
        _expandingTabIndex.value = model.expandingController?.value??0;
      }
    });
  }
